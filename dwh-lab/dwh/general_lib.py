...
def read_azure_datalake_storage(file_system_name, file_path, account_name, account_key):
    ...
    download = file_client.download_file(timeout=300)
    file_content = download.readall().decode('utf-8') 
...
def send_email(email_recipient, email_subject, email_body_text):
    # ========== 🔧 CONFIGURATION ==========
    your_email = "trang.pham@jaist.ac.jp"
...
def read_chunk_and_writle_dls(source_file_system_name, source_file_path, destination_file_system_name, destination_file_path,
    ...
    for chunk in chunks:
        ...
        archive_file_client.append_data(data=parquet_data, offset=0,  length=len(parquet_data))
        archive_file_client.flush_data(len(parquet_data))
...
def write_dls(input_df, format, account_name, account_key, file_system_name, file_path, chunk_size=100000):
